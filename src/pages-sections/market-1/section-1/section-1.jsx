  ...
  return <div className="bg-white mb-4">
      <Container className="pt-2 pb-2">
        <Carousel slidesToShow={1} arrows={false} dots autoplay>
          {carouselData.map((item, ind) => <CarouselCard1 key={ind} title={item.title} imgUrl={item.imgUrl} buttonLink={item.buttonLik} buttonText={item.buttonText} description={item.description} />)}
        </Carousel>
      </Container>
    </div>;
}