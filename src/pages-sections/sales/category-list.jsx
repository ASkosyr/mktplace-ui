...
const CategoryList = forwardRef(({
  ...
  return <Box mb={4} overflow="hidden" ref={ref}>
        <FlexBox m={-1.5} flexWrap="wrap">
          {categories.map(item => {
        const selectedItem = item.slug === params.slug ? 1 : 0;
        return <CategoryBoxWrapper key={item.slug} selected={selectedItem} onClick={() => router.push(`/sales-1/${item.slug}`)}>
                <IconComponent icon={item.icon} fontSize="inherit" sx={{
            fontSize: 44
