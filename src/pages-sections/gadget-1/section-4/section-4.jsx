  ...
  return <div className="mt-4">
      <Container>
        ...
        backgroundColor: "white"
          ...
          {products.map(item => <ProductCard2 key={item.id} slug={item.slug} title={item.title} price={item.price} off={item.discount} rating={item.rating} imgUrl={item.thumbnail} />)}
        </Carousel>
      </Container>
    </div>;
}