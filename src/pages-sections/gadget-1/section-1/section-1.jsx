...
export default async function Section1() {
  const [products, mainCarousel] = await Promise.all([api.getTopPicksList(), api.getMainCarousel()]);
      ...
      <Grid container spacing={5}>
        <Grid item md={5} xs={12}>
          ...
          background: "transparent"
            ...
            {mainCarousel.map(product => <CarouselCard3 key={product.id} buttonText="Shop Now" title={product.title} imgUrl={product.thumbnail} url={`/products/${product.slug}`} description={`Starting at $${product.price} & save upto ${product.discount}%`} />)}
          </Carousel>
        </Grid>
